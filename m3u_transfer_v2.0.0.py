...
def sync_one(source: str, target: str, time_offset_hours: float) -> SyncResult:
...
def copy_from_m3u(
    ...
    if workers > 1 and total_tracks > 1:
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futures = [pool.submit(sync_one, source, target, time_offset)
                       for source, target in files_to_copy]
            for future in as_completed(futures):
                handle(future.result())
    else:
        for source, target in files_to_copy:
            handle(sync_one(source, target, time_offset))
