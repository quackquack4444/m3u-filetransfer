...
def case_key(path: str, ignore_case: bool) -> str:
    """
    Key used to compare paths for "is this the same file" purposes.

    Normally this just follows the OS's own rules (case-folded on Windows,
    exact on Linux/most SD cards). With `ignore_case=True`, casing is folded
    unconditionally, so e.g. "Track1.mp3" and "track1.mp3" are treated as the
    same file no matter what platform or filesystem is involved.
    """
    return path.lower() if ignore_case else os.path.normcase(path)


@dataclass
class DestinationIndex:
    """Snapshot of what's already on disk, taken before any copying happens."""
    extra_files: list[str]
    existing_files: dict[str, str]  # case_key(path) -> actual on-disk file path
    existing_dirs: dict[str, str]   # case_key(path) -> actual on-disk directory path
...
def scan_destination(
    ...
    """
    Walk `destination` once and index what's there:
      - extra_files: absolute file paths not present in `expected`
      - existing_files / existing_dirs: case_key(path) -> actual on-disk path,
        for every file and every directory found. Used afterwards so a
        pre-existing, differently-cased file or folder gets reused instead of
        being duplicated under a freshly-cased name.
    """
    extras: list[str] = []
    existing_files: dict[str, str] = {}
    existing_dirs: dict[str, str] = {case_key(destination, ignore_case): destination}
    scanned = 0

    for root, _dirs, files in os.walk(destination):
        existing_dirs[case_key(root, ignore_case)] = root

        for filename in files:
            full_path = os.path.abspath(os.path.join(root, filename))
            scanned += 1

            key = case_key(full_path, ignore_case)
            existing_files[key] = full_path
    ...
    return DestinationIndex(extras, existing_files, existing_dirs)
...
def resolve_target_path(
    ...
    """
    Resolve a playlist entry's relative path against what's already on disk,
    one path segment at a time, so it lands inside pre-existing folders (and
    matches a pre-existing file) even when only the letter case differs -
    e.g. a new track for "Tool" lands inside an existing "TOOL" folder rather
    than a fresh "Tool" folder being created alongside it.

    Segments with no existing case-insensitive match keep their original
    (playlist-derived) casing, since there's nothing on disk yet to match.
    """
    parts = relative.split(os.sep)
    current_dir = destination

    for part in parts[:-1]:
        candidate = os.path.join(current_dir, part)
        current_dir = index.existing_dirs.get(case_key(candidate, ignore_case), candidate)

    candidate_file = os.path.join(current_dir, parts[-1])
    return index.existing_files.get(case_key(candidate_file, ignore_case), candidate_file)
...
def copy_from_m3u(
    ...
    # -- Pass 1: read the playlist, resolve every entry to a naive absolute target --
    relative_pairs = read_playlist(m3u_file, source_base)
    naive_targets = [
        os.path.normpath(os.path.join(destination, relative))
        for _source, relative in relative_pairs
    ]
    expected = {case_key(target, ignore_case) for target in naive_targets}

    total_tracks = len(relative_pairs)
    print(f"\nPlaylist contains {total_tracks:,} track(s).")
    if time_offset:
        print(f"Timestamp comparison offset: {time_offset:+g} hour(s)")
    if ignore_case:
        print("Case differences in filenames and folder names will be ignored.")
    ...
    index = scan_destination(destination, expected, ignore_case)
    if index.extra_files:
        prompt_delete_extras(index.extra_files)
    else:
        print("\nNo files in the destination need removing.")

    # Resolve each playlist entry against what's already on disk - so a track
    # for a pre-existing, differently-cased folder lands inside that folder
    # (and a pre-existing, differently-cased file gets updated in place)
    # instead of a freshly-cased duplicate being created.
    files_to_copy = [
        (source, resolve_target_path(destination, relative, index, ignore_case))
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--workers", type=int, default=1, metavar="N",
                         help="Copy this many files concurrently. Try 4-8 for network drives "
                              "or many small files. Leave at 1 for spinning hard drives, SD "
                              "cards, or USB flash drives - concurrent writes to these turn "
                              "sequential I/O into contention and usually make things slower. "
                              "Default: 1 (serial, matches original behavior)")
    ...
    parser.add_argument("--ignore-case", action="store_true",
                         help="Treat filenames that only differ by letter case as the same file "
                              "(e.g. 'Track1.mp3' and 'track1.mp3'), so a pre-existing file isn't "
                              "flagged as extra or duplicated under a different-cased name. Off "
                              "by default, matching the original, platform-dependent behavior.")
